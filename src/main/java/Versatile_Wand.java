...
/** An ImageJ magic wand with selectable tolerance, variable hue or
  ...
  *
  * Version 1.14, Michael Schmid, 2012-Dec-09: Again Bugfix: RGB tolerance was inconsistent
  * Version 1.13, Michael Schmid, 2012-Dec-07: Bugfix RGB tolerance was incorrect (incomplete)
...
//}
    ...
    boolean checkColor(int rgb, int r0, int g0, int b0, double rWeight, double gWeight, double bWeight) {
        ...
        double deltaSqr = (1./3.)*(deltaR*deltaR + deltaG*deltaG + deltaB*deltaB);
//if(temmmmp==253740)IJ.log("cSens="+(float)colorSensitivity+" dR="+deltaR+" dG="+deltaG+" dB="+deltaB);
//if(temmmmp==253740)IJ.log("rWeight="+rWeight+" gWeight="+gWeight+" bWeight="+bWeight);
...
//if(temmmmp==253740)IJ.log("deltaSqr="+(float)deltaSqr+" deltaParSqr="+(float)deltaParSqr+" cSens="+(float)colorSensitivity);
            if (colorSensitivity <0)
